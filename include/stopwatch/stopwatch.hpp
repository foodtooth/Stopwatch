#ifndef INCLUDE_STOPWATCH_STOPWATCH_HPP_
#define INCLUDE_STOPWATCH_STOPWATCH_HPP_
...
class Stopwatch {
 public:
  ...
  Stopwatch() : start_time(), laps({}) { start(); }
  ...
  template <TimeFormat fmt_total = TimeFormat::MILLISECONDS,
            TimeFormat fmt_lap = fmt_total>
 ...
 private:
  ...
  template <TimeFormat fmt = TimeFormat::MILLISECONDS>
  static std::uint64_t ticks(const time_pt &start_time,
                             const time_pt &end_time) {
    ...
    switch (fmt) {
      ...
      case TimeFormat::MICROSECONDS: {
        ...
        const auto us_count = (ns_count / 1000) + up;
        return us_count;
      }
...
constexpr Stopwatch::TimeFormat us = Stopwatch::TimeFormat::MICROSECONDS;
...
constexpr Stopwatch::TimeFormat nanoseconds =
    Stopwatch::TimeFormat::NANOSECONDS;
constexpr Stopwatch::TimeFormat microseconds =
    Stopwatch::TimeFormat::MICROSECONDS;
constexpr Stopwatch::TimeFormat milliseconds =
    Stopwatch::TimeFormat::MILLISECONDS;
...
#endif /* INCLUDE_STOPWATCH_STOPWATCH_HPP_ */
